functions {
	...
	real Ac1  = z[7];  // cumulative sintomatics cases novac
	...
	// second layer: vaccinated people
       ...
       //function f_V(t)
        ...
        //update first layer
            ...
            // cumulative sintomatics cases novac
            subr*((1-pN)*LambdaN+ subr*(1-pV)*LambdaV),
...
data {
    ...
    // observed data
    real yIs[N]; // cumulative number of sintomaticos infectious peopole novac
    real yD1[N]; // deaths novac
    real yD2[N]; // deaths vac    
...
real<lower=0,upper=1> sigma; //se usa en la estimación de las cond iniciales
real<lower=0> v;
...
//real S0N=N0-I_s0N-I_a0N-R_a0N-R_s0N-D0N-S0V-I_s0V-I_a0V-R_a0V-R_s0V-D0V;
real theta[11] ={gamma_aN, gamma_sN, deltaN, beta_aN, beta_sN, gamma_aV, gamma_sV, deltaV, beta_aV, beta_sV,subr};

real z[N,13];

z[1]={S0N,I_a0N,I_s0N,R_a0N,R_s0N,D0N,Ac0,S0V,I_a0V,I_s0V,R_a0V,R_s0V,D0V};
...
model {
 ...
 I_a0V ~ lognormal(log(I_a0V_anterior), sigma);
 I_s0V ~ lognormal(log(I_s0V_anterior), sigma);
 R_a0V ~ lognormal(log(R_a0V_anterior), sigma);
 Ac0 ~ lognormal(log(yIs[1]), sigma);
 D0N ~ lognormal(log(yD1[1]), sigma);
 D0V ~ lognormal(log(yD2[1]), sigma);
 // subreporte
 subr ~ beta(2,2);
  ...
  yD1  ~ lognormal(log(z[:,6]), sigma);
  yD2  ~ lognormal(log(z[:,13]), sigma);
  yIs ~ lognormal(log(z[:,7]), sigma);

}

generated quantities {
    real y_pred[N_pred,13];
    y_pred[1,] = {S0N,I_a0N,I_s0N,R_a0N,R_s0N,D0N,Ac0,S0V,I_a0V,I_s0V,R_a0V,R_s0V,D0V};
