...
D0N_anterior = float(inputfl['D0Nf'])
R_s0V_anterior = float(inputfl['R_s0Vf'])
D0V_anterior = float(inputfl['D0Vf'])

S0N_anterior = float(inputfl['S0Nf']) # S0 final del mes anterior (15/08) no vacunados
S0V_anterior = float(inputfl['S0Vf']) # S0 final del mes anterior (15/08) vacunados #float(inputfl['S0Vf'])

#S0Vr_anterior = float(inputfl['S0Vrf']) # S0 final del mes anterior (15/08) vacunados #float(inputfl['S0Vf'])

S0Vr_anterior = 260246 # vacunos con refuerzo al 15/08

#S0Vr_anterior = float(inputfl['S0Vrf'])
#I_a0Vr_anterior = float(inputfl['I_a0Vrf'])
#I_s0Vr_anterior = float(inputfl['I_s0Vrf'])
#R_a0Vr_anterior = float(inputfl['R_a0Vrf'])
#R_s0Vr_anterior = float(inputfl['R_s0Vrf'])
#D0Vr_anterior = float(inputfl['D0Vrf'])
...
fit = sm.sampling(data=stan_data, iter=200, warmup=100, chains=10, check_hmc_diagnostics=False, control={'max_treedepth': 11})
...
params = ['gamma_aVr', 'gamma_sVr','deltaVr','beta_aN','beta_sN','beta_aV','beta_sV','beta_aVr','beta_sVr','subr','S0N','I_a0N','I_s0N','R_a0N','R_s0N','D0N','Ac0','S0V','I_a0V','I_s0V','R_a0V','R_s0V','D0V','D0','gamma_aN','gamma_sN','deltaN','gamma_aV','gamma_sV','deltaV','SNf','I_aNf','I_sNf','R_aNf','R_sNf','SVf','I_aVf','I_sVf','R_aVf','R_sVf','SVrf','I_aVrf','I_sVrf','R_aVrf','R_sVrf','DNf','DVf','DVrf','Df']
